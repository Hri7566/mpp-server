...
class Channel extends EventEmitter {
    ...
    constructor(server, _id, settings, cl) {
        ...
        if (this.isLobby(this._id)) {
            this.settings = new ChannelSettings(this.server.lobbySettings);
            ...
            this.settings = new ChannelSettings(settings, "user");
        }
        ...
        if (this._id == LOGGING_CHANNEL) {
            if (cl.user.hasFlag("admin")) {
                ...
                this.settings = ChannelSettings.changeSettings(
                    {
                        color: c.toHexa(),
                        color2: c2.toHexa(),
                        chat: true,
            ...
            Database.getChannelSettings(this._id, (err, set) => {
                if (err) {
                    return;
                }
    ...
    join(cl, set) {
        ...
        if (!otheruser) {
            ...
            if (
                ...
                this.crowndropped = false;
                // this.settings = new ChannelSettings(set, 'user');
            } else {
                        ...
                        if (typeof set == "undefined") {
                            if (typeof this.settings == "undefined") {
                                this.settings = new ChannelSettings(
                                    this.server.defaultChannelSettings,
                                    "user"
                                );
                            } else {
                                this.settings = new ChannelSettings(
    ...
    verifySet(_id, msg) {
        ...
        msg.set = ChannelSettings.changeSettings(msg.set);
    ...
    setData() {
        Database.setChannelSettings(this._id, this.settings, this.chatmsgs);
    }
